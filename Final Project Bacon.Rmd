...
#This grabs all the action movies since 2000
...
#This is a function to download all action movies from a given year
get_year_movies <- function(year_str) {

  #This gets the first page of results for this year.
  ...
  #This is how many pages we are requesting.
  total_pages <- min(first$total_pages, 500)

  #This is looking through all the pages and making them into a data frame.
...
#This combines all the action movies into one data frame.
...
```{r}
# This function finds the shortest actor–movie–actor path
# from a given actor to Mark Wahlberg using a specified graph.
# It returns both the path and the number of movies involved.
...
# This function creates a restricted version of the full actor graph.
# It removes all movie nodes that do NOT meet a given condition
# (e.g., only popular movies), while keeping all actor nodes.
...
```{r}
# Define a cutoff for "popular" movies
# Here, we keep only movies in the top 10% of popularity
...
```{r}
# This function summarizes key structural properties of a graph.
# These metrics help compare the full graph to the restricted graph.
...
graph_summary <- function(g) {
  ...
  # Return important network statistics
  data.frame(
    vertices          = vcount(g),            # total nodes
    edges             = ecount(g),             # total connections
    components        = comps$no,               # number of disconnected groups
    largest_component = max(comps$csize),      # size of biggest group
...
```{r}
# Summary statistics for the full actor network
graph_summary(actor_graph)

# Summary statistics for the popular-movies-only network
graph_summary(popular_graph)
```
