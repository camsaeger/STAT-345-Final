...
### Subset Actor Connectedness Graph 
...
### Creating Modified Function

# Here we are modifying our previous function to take into account only the most popular movies.

connect_to_mark_2 <- function(actor, graph, actor_movies) {

  # This computes the shortest path between the actor and Mark Wahlberg
  p <- shortest_paths(
    graph,
    from = actor,
    to   = "Mark Wahlberg"
  ...
  # This extract the node names along the path
  nodes <- names(p)

  # This replaces movie IDs with movie titles 
  path <- sapply(nodes, function(x) {
  ...
  # This counts how many movie nodes appear in the path
  movie_count <- sum(path %in% actor_movies$title)

  # This returns the path and movie count
  list(
    path   = unname(path),
    movies = movie_count
  )
...
```{r}
### Creating Modified Graph

# Here we are creating the connectedness graph of our modified function based on popularity.

make_restricted_graph <- function(full_graph, allowed_movie_ids) {

  # This keep all actor nodes and only the movie nodes that meet the restriction
  keep_names <- c(
    V(full_graph)$name[!(V(full_graph)$name %in% actor_movies$id)],
    allowed_movie_ids
  )

  # Here we induce a subgraph containing only the kept nodes
  induced_subgraph(
    full_graph,
...
# Then we identify movie IDs that meet the popularity requirement
allowed_movies <- actor_movies$id[actor_movies$popularity >= cutoff]

# Then we created a new graph that only includes popular movies
...
# Finally we test our modified function with top popular movies
